...
#if 1
    rc |= csv_find("scl",    &sample.i2c_clk);
    rc |= csv_find("sda",    &sample.i2c_sda);
#else
    rc |= csv_find("scl2",    &sample.i2c_clk);
    rc |= csv_find("sda2",    &sample.i2c_sda);
#endif
...
static void event(event_e event, time_nsecs_t time_nsecs)
{
	...
	if (event == EVENT_CLK_RISE) {
		...
		hdr(lf, time_nsecs, "PARSER_I2C");
		fprintf(lf->fp, "I2C Address: %x ", accumulated_byte);
		state = STATE_RW_BIT;
	    }
	}
	break;
	...
	if (event == EVENT_CLK_RISE) {
	    ...
	    fprintf(lf->fp, "[ %s ] ", rw == DIR_RD ? "Rd":"Wr");
	...
	if (event == EVENT_CLK_RISE) {
	    ...
	    st25dv_i2c((i2c_address << 1) | rw, time_nsecs);
   ...
   case STATE_COLLECT_DATA:
	   ...
	   if ( accumulated_bits == 8 ) {
	       ...
	       fprintf(lf->fp, "%02x ", accumulated_byte);
		...
		*/
	       ...
	       st25dv_i2c(accumulated_byte, time_nsecs);
